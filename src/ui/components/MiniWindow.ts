...
export class MiniWindow {
  ...
  private toolStatusContainer: HTMLElement | null = null; // Renamed from toolStatus
  private answerContainer: HTMLElement | null = null;
  private currentToolStep: string = '';
  private steps: { id: string; element: HTMLElement; status: 'running' | 'completed' | 'error' }[] = []; // Store step elements
   ...
   */
  public addToolStep(text: string): string {
    if (!this.toolStatusContainer) return '';
   ...
   */
  public updateToolStepStatus(stepId: string, status: 'completed' | 'error', newText?: string): void {
   ...
   */
  public startProcessing(question: string): void {
