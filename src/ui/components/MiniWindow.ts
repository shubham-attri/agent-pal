...
export class MiniWindow {
  ...
  public isVisible: boolean = false; // Made public for ChatBar check
  private processingQuestion: boolean = false;
  private toolStatus: HTMLElement | null = null;
  private answerContainer: HTMLElement | null = null;
  private currentToolStep: string = '';
  ...
  private onExpandCallback: ((question: string, answer: string) => void) | null = null;
   ...
   */
  public setupExpandAction(callback: (question: string, answer: string) => void): void {
   ...
   */
  private addToolStep(text: string, status: 'active' | 'completed' | 'error' | 'pending' = 'pending'): HTMLElement {
    if (!this.toolStatus) return document.createElement('div'); // Should not happen
   ...
   */
  private updateLastToolStep(status: 'active' | 'completed' | 'error'): void {
    if (!this.toolStatus || !this.toolStatus.lastElementChild) return;
    const lastStep = this.toolStatus.lastElementChild as HTMLElement;
   ...
   */
  public async processQuestion(question: string): Promise<void> {
    if (this.processingQuestion || !this.container) return;
   ...
   */
  public getLastInteraction(): { question: string, answer: string } {
